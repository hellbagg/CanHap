...
void setup(){
   ...
   */ 
  haplyBoard          = new Board(this, "/dev/cu.usbmodem1422301", 1);
...
while(1==1) {
    ...
    iter+= 1;
    println("P: "+ P + " I: " + I + " D: " + D);
    println("smoothing: "+ smoothing + " looptime: " + looptime);
...
//Loop to trace and move to next target point 
    ...
    if (abs(dist_X) < 40 && abs(dist_Y) < 40 && xr >= (savexr + 0.5)){
      ...
      //println("xr: " + xr + " " + "yr: " + yr);
    if (yr >= (saveyr + 0.5) && xr >= (savexr + 0.5)){
      xr = savexr;
      yr = saveyr;
    }
    
    //draws a box but trying to figure out how to make the tracking work.
    //if (yr >= (saveyr + 0.5)){
    //   yr = saveyr + 0.5;
    //   xr -= 0.01;
    // }
